...
/**
 * The close button renders a lucide COMPONENT, not markup. The previous dashboard passed SVG
 * source strings into a DOM helper that escaped them, so the modal printed
 * `<svg width="16" ...>` on screen. A component makes that class of bug unrepresentable.
 */
...
function DialogContent({ className, children, ...props }: React.ComponentProps<typeof DialogPrimitive.Content>) {
  return (
    <DialogPortal>
      ...
      >
        {children}
        {/* rounded-sm inside a p-6 rounded-xl surface: nowhere near the corner, so no concentricity to match. */}
